        ...
        pause1.setOnFinished(e1 -> {
            ...
            pause2.setOnFinished(e2 -> {
                ...
                pause3.setOnFinished(e3 -> {
                    ...
                    this.state.set(GCState.COMPACTING);
                    List<AtomicReference<Obj>> liveObjectsInHeap = heap.stream().filter(ref -> CellState.REFERENCED == ref.getOpaque().getState()).toList();
                    int liveObjectsCounter = 0;
                    if (!liveObjectsInHeap.isEmpty()) {
                        System.out.println("Compact live objects");
                        for (int y = 0; y < heap.getNoOfRows(); y++) {
                            for (int x = 0; x < heap.getNoOfCols(); x++) {
                                if (liveObjectsCounter < liveObjectsInHeap.size()) {
                                    heap.getItemAt(x, y).update(liveObjectsInHeap.get(liveObjectsCounter).get());
                                    liveObjectsCounter++;
                                } else {
                                    heap.getItemAt(x, y).setState(CellState.FREE);
                                }
                            }
                        }
                    }
